...
function Profile() {
  ...
  if (error) return <p className="text-red-500">Error fetching profile: {error.message}</p>;
        ...
        <div className="custom-container py-5">
          <div className="flex space-x-8 lg:px-5">
            ...
            <div className="flex-1 rtl:pr-8">
              <h1 className="text-xl font-medium text-gray-700 dark:text-white">{profile.username}</h1>
              <p className="text-sm font-medium text-gray-500 dark:text-gray-500">{profile.email}</p>
            </div>
          </div>
        ...
        <div className="custom-container">
          <div className="flex space-x-8">
            <div className="flex-1">
              <h3 className="text-xl font-medium text-gray-700 dark:text-white mb-5">Activity</h3>
            ...
            <div className="max-w-sm w-full">
              <div className="bg-white border border-gray-200 dark:border-gray-900 shadow-sm dark:bg-gray-800 p-8 rounded-xl mb-4">
                <h3 className="font-medium text-gray-700 dark:text-white mb-5">Overview</h3>
                <hr className="my-4 lg:my-6 border-gray-100 dark:border-gray-700/50" />
                {[
                  { label: "Content watched", value: "0" },
                  { label: "Member for", value: profile.member_since },
                  { label: "Content liked", value: "0" },
                ].map(({ label, value }) => (
                  <div key={label} className="flex items-center mb-5 space-x-8">
                    <div className="flex-1">
                      <div className="text-xs text-gray-500 dark:text-gray-400">{label}</div>
                      <div className="text-base font-medium text-gray-700 dark:text-gray-100">{value}</div>
                    </div>
                  </div>
                ))}
              </div>
